#!/bin/bash
# Inicialización de BD — se ejecuta en background al arrancar el contenedor app
...
for i in $(seq 1 30); do
  if mysqladmin --ssl-mode=DISABLED ping -h"$HOST" -uroot -p"$ROOT_PASS" --silent 2>/dev/null; then
...
for SQL in /var/www/html/sql/0*.sql; do
  ...
  $MYSQL -h"$HOST" -u"$USER"  -p"$PASS"   "$NAME" < "$SQL" 2>&1 || true
done
